...
if [ -f "./backup/mysql.sql.gz" ]; then
    bash ./docker/util/message.sh warning "Attention!"
...
if [ "$backupOK" = "Yes" ] || [ "$backupOK" = "yes" ] || [ "$backupOK" = "Y" ] || [ "$backupOK" = "y" ]; then
    bash ./docker/util/message.sh info "Creating a backup of your database in backup/mysql.sql.gz"
    mkdir -p backup
    ...
    bash ./docker/util/message.sh success "Backup successful!"
fi
